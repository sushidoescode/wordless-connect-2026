...
| Freeze integrity at candidate bind | No tracked runtime/UI-manifest file changed after the freeze commit; re-verified when the release candidate is bound | PENDING | — |
...
| Gate | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| Clean worktree | Worktree clean at recording | PENDING | — |
...
| --- | --- | --- | --- |
| Duration | At least 58.5 and below 60.000 seconds; target 59.000 | PENDING | — |
| Video stream | Sole video stream H.264, 1920×1080, yuv420p, square-pixel, 60/1 fps | PENDING | — |
| Audio stream | AAC at 48 kHz | PENDING | — |
| Frame count | Decoded frame count matches the declared 60 fps duration exactly | PENDING | — |
| Decode | Clean full ffmpeg decode with no error | PENDING | — |
| Faststart | Top-level MP4 atom order places `moov` before `mdat` | PENDING | — |
| Loudness | Integrated loudness within −17 to −15 LUFS (target −16); true peak no higher than −1.5 dBTP | PENDING | — |
| Black intervals | `blackdetect` finds no unplanned black interval of 0.25 s or longer | PENDING | — |
| Silent tail | `silencedetect` finds no unplanned tail of 0.50 s or longer | PENDING | — |
| Hash | Final candidate SHA-256 recorded here and in the EDL | PENDING | — |
...
| Playback inspection | Watched end to end at 1440×810 laptop playback and at 390 px phone width | PENDING | — |
| Verifier | `tools/media/verify-wordless-video.mjs` passes as the single checked entry point; parsed measurements recorded here | PENDING | — |

## 4. Frame and story gates (per `docs/submission/video-edl.md`)

| Gate | Requirement | Status | Observed |
| --- | --- | --- | --- |
| 00.0 open | Split-screen roles, active shared stroke, `LENS STUDIO PREVIEW — SIMULATED` disclosure at frame zero | PENDING | — |
| 04.0–09.0 | Uncut browser wrong action; bilateral coral stroke response; caption exactly `WRONG · BOTH STROKES GO CORAL` | PENDING | — |
| By 14.0 | Correct state, mint feedback, and revealed word visible | PENDING | — |
| Before 20.0 | Both exact-path glyph medallions complete and holding | PENDING | — |
| 20.0–26.0 | Clean reset and palette beat | PENDING | — |
| Split screen | Lens/browser split continuously visible 00.0–51.0; both product panes remain visible (insets acceptable) during CLAD/validation beats; 51.0–59.0 end card sole full-frame exception | PENDING | — |
| Disclosure ranges | `LENS STUDIO PREVIEW — SIMULATED` readable on every Preview-bearing segment; observed ranges recorded in the EDL | PENDING | — |
| End card | 51.0–59.0 original end card, complete audio, no clipped copy | PENDING | — |

## 5. Complete-cut two-stage panel (amendment §5)

| Gate | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| Panel composition | Three fresh, context-isolated multimodal reviewers; one fixed two-stage prompt; verbatim answers stored under `local-handoff/reviews/task16/` (ignored) | PENDING | — |
| Stage A (blind, muted) | Each reviewer receives only the complete final MP4, muted, neutral questions, cited timestamps; at least two of three independently identify: one side draws and the other guesses; the browser guess changes the shared result; the exact shared mark becomes the glyph | PENDING | — |
| Stage A freeze | Timestamped Stage A answers frozen and recorded before any supporting text is revealed | PENDING | — |
| Stage B (claims) | Same reviewers then receive the intended audio version, project description, README first screen, and claim ledger; assess theme fit, role clarity, browser causality, magic-moment timing, spatial necessity, visual finish, CLAD proof, cross-document consistency, and unsupported claims | PENDING | — |
