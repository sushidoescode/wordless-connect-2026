...
| Gate | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| Core + web + build | `npm run check` passes | PASS | core 363 tests, web unit 109, `tsc` clean, vite build 55 modules / 253 KB (2026-08-30) |
...
| Gate | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| Tracked-tree scan | Tracked-tree and prospective-archive secret scans clean | PASS | `tools/security/audit-prospective-tree.mjs` on the prospective tree: 0 env-value leaks, 0 blocking findings; the 19 secret-shaped hits are all the auditors' own patterns / synthetic fixtures / the plan's documented commands (enumerated). Tree IDs + reports under ignored `captures/task16/`. |
...
| Narration sidecars | Ignored narration/candidate sidecar checks, including an in-memory exact-byte comparison against the ignored environment value without printing it | PASS | the wrapper reads `.env` / `web/.env.local` in memory and calls `auditReleaseHistory` in-process (env values never serialised); `env-value-leak` compared against every blob → 0 hits. Verified separately that no env value appears in any generated report. |
