...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 00.0–04.0 | Immediate split-screen roles, shared live line, four choices | `ONE DRAWS IN SPACE · ONE GUESSES IN A BROWSER` (runtime `PAINTER` / `GUESSER` labels visible) | `LENS STUDIO PREVIEW — SIMULATED` from frame zero | ~00.0–04.0 | (recut) Split screen, `PAINTER`/`GUESSER` roles, four neutral cards, shared live stroke, disclosure top-left all present from frame 0. Shipped beat-1 burned caption: `One draws in space — a friend guesses in a browser.` |
...
| 3 | 09.0–14.0 | Browser chooses correctly; mint response and word reveal (uncut browser action; correct feedback and revealed word visible by 14.0) | `CORRECT · THE WORD IS REVEALED` | `LENS STUDIO PREVIEW — SIMULATED` | ~09.0–14.0 | (recut) `✓ STAR · CORRECT` (mint) and caption `CORRECT · the word is revealed` visible ~10.3 s (caption onset == visual onset) — well before the 14.0 s gate. |
| 4 | 14.0–20.0 | Exact sampled path locks as matching glyph medallions, holding before 20.0 | `THE EXACT PATH BECOMES THE GLYPH · NO RECOGNITION, NO REDRAW` | `LENS STUDIO PREVIEW — SIMULATED` | ~14.0–20.0 | (recut) Matching glyph medallions on both surfaces (present from ~10.3 s, holding through 20.0 s). Shipped burned caption `The exact shared path becomes their glyph.`; the planned `NO RECOGNITION, NO REDRAW` tag ships later on the ~42 s honesty card — see reconciliation. |
| 5 | 20.0–26.0 | Replay/reset and a concise palette beat (violet / lemon / mint selectable; clean successor round) | `PLAY AGAIN · THE PAINTER PICKS A NEW STROKE COLOR` | `LENS STUDIO PREVIEW — SIMULATED` | ~20.0–26.0 | (recut) Reset + fresh round; shipped caption `Play again — a fresh round begins.` retimed to the beat (visible from 20.0 s). The planned "new stroke color" line was dropped — the captured replay reused violet; see reconciliation. |
...
| 8 | 51.0–59.0 | Original end card and one-sentence promise; sole full-frame exception; complete audio, no clipped copy | `WORDLESS RELAY · DRAW IN SPACE · GUESS IN A BROWSER · KEEP THE MARK YOU MADE TOGETHER`; end card carries the `LENS STUDIO PREVIEW PROTOTYPE — SIMULATED` line | Disclosure printed on the end card itself | 51.0–59.0 | (recut) Full-frame end card (sole full-frame exception): the body/end-card concat cut lands at 51.0 s; `WORDLESS RELAY / Draw in space. Guess from a browser. Keep the mark you made together. / Lens Studio Preview prototype — simulated`, holding to the tail; no clipped copy. |

The Observed columns above reflect the **recut candidate** `254945f2…` (marked
"(recut)" where they differ from the planned column); the "Recut reconciliation"
section further down records the full set of recut deviations.
