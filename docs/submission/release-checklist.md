...
| Gate | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| Core + web + build | `npm run check` passes | PASS | core 424 tests, web unit 109, `tsc` clean, vite build 55 modules / 253 KB (2026-08-30; +13 parser omitted-vs-empty / duplicate-singleton-flag regressions this pass) |
...
| Two-pass prospective audit | `tools/security/audit-release-history.mjs` on the preliminary and final prospective trees; exit codes: 0 clean, 10 only the exact known blockers, 20 new/blocking finding, 30 audit error; only 0 or 10 may continue to a private local candidate, and 10 still blocks publication | PASS | two-pass run via the safe wrapper `tools/security/audit-prospective-tree.mjs` (calls `audit-release-history.mjs` in-process): preliminary prospective tree and committed HEAD tree both exit 0 (clean of real findings); committed HEAD tree proven identical to the audited candidate tree. Tree IDs + reports preserved under ignored `captures/task16/`. Wrapper logic — fail-closed partition, content-based whole-buffer text detection, fail-closed `ls-tree -z` / object-id retrieval (unsafe filename, malformed/empty record, non-NUL-terminated output, unknown/malformed header, non-blob gitlink terminal entry, and cat-file/sidecar read failure all → exit 30, never a silent empty), and fail-closed explicit inputs (a missing/non-directory/unreadable `--sidecars` dir, an unsupported entry inside it, or a supplied `--root-env`/`--web-env` that is missing, non-regular, binary, or invalid UTF-8 all → exit 30; an omitted env/sidecar option stays valid) and fail-closed argument parsing (an explicitly empty flag value `""` and a repeated singleton flag `--tree`/`--root-env`/`--web-env`/`--report`/`--tree-id-out` both → exit 30, while `--sidecars` may repeat and omitted optional flags stay valid) — covered by `tools/core-tests/prospective-tree-audit.test.mjs` (70 tests). |
